 ...
 * be read afterwards: open /?crash=1 (or the /me page) to see it. Nothing leaves the device.
 */
export function CrashLog() {
  useEffect(() => {
    ...
    const onErr = (e: ErrorEvent) => save("error", e.message, e.error?.stack);
    const onRej = (e: PromiseRejectionEvent) => save("rejection", String(e.reason?.message ?? e.reason), e.reason?.stack);
