...
export const GET = route({ query: SearchQuery, limit: "embed" }, async ({ uid, ip, query }) => {
  // Over budget → lexical only, not a refusal: search must keep working, it just stops calling the embedder.
  let semantic = true;
  if (query.q) { try { await charge("embed", { uid, ip }); } catch (e) { if (e instanceof HttpError && e.status === 503) semantic = false; else throw e; } }
