...
This work was done after the prototype worked, in three passes, each in the order that made the next step safe: tests before refactoring, validation before sharing the URL, the read model before growing the corpus, and then a review of the whole codebase as if by a stranger, which is what found the two holes named below. Two facts drove most of it. Redis is 75 ms away, so a single round trip costs what a whole feed should, and every path was rebuilt to one hop. And the corpus is about to grow without a ceiling, because backfill adds repos behind every novel deep dive, so anything linear in corpus size was a cliff in waiting.

**It fails safely.** One guard sits on every route: user or 401, a schema per query, body and path that names the offending field, 413 on oversize, 400 on bad JSON, a 403 when a mutation's Origin names another host, and a 500 that never leaks the message. The session cookie is signed, so the server only ever acts on ids it minted; a tampered one is replaced, not trusted. Every setting is read in one typed place, and boot names the missing keys. Redis is bounded to five retries and a five-second connect, and a corrupt row costs one item rather than the cache every user shares. Every outbound call has a deadline shorter than the function that makes it: the model SDKs defaulted to ten minutes, which outlived the sixty-second function that called them while the tokens still billed. There is one client fetch: a non-2xx becomes a human message with a request id, reads retry once, and every failure has a visible state and a retry, so there is no spinner forever.

**It cannot be made expensive.** Every route that can spend money is behind two ceilings. Per-caller rate limits, sized by what the route costs us rather than how often a human would call it, and with an IP ceiling on every bucket so dropping the cookie does not reset them. And one global daily budget in Redis, charged before the call, so the sum is bounded and not just each caller: at the old per-user limits one address could have run $2,600 of Haiku in a day. When the budget is gone, search falls back to lexical, voice answers from the brief, and backfill waits for tomorrow; nothing refuses outright that can degrade instead. Fetching a repo nobody has heard of is a 404 before it is a GitHub call, so walking `/api/items/*` with a wordlist costs nothing. The server-rendered repo page had skipped that gate entirely — the review found it — and now shares it. A reaction is idempotent, so a replayed request cannot compound a profile. Both limiters fail open: a down cache must not take the read path with it.

**It knows what it keeps.** An anonymous profile lives as long as its cookie plus slack, refreshed on every visit, with the seen set capped at five thousand; before this every cookie ever minted left its keys in Redis forever. The voice trace, which exists because the call is browser-to-OpenAI and leaves no server evidence, records transport events and tool names, but the user's words are reduced to their length and the microphone's device name is not taken. Free text the user types is bounded and stripped of control characters at the door, and the error page shows a stack only in development. The browser is told the same things in headers: scripts only with this response's nonce, connections only to us and to OpenAI, no framing, the microphone only for this origin. The nonce is why the design document renders per request rather than at build.

**It can be seen.** Server 500s, client throws, and the two server failures that never surface as an error (a voice tool throwing, the realtime handshake refused) ship to a ring buffer with per-day fingerprints and the request id that joins a client symptom to a server log. Every route records its latency into fixed buckets, one fire-and-forget pipeline per request, and `/api/health` reports Redis latency, corpus version, today's spend, the backfill queue, and per-route p50/p95/5xx for the day. A webhook is paged, at most once an hour per kind, when the spend cap refuses or one error fingerprint passes twenty-five in a day.
...
**It can be changed.** There are 258 tests against a real scratch Redis rather than a mock, covering ranking, search fusion, user state, embeddings, the read-model snapshot, related-project scoring, the guard, rate limits, the spend cap, the backfill queue, on-demand fetch deduplication, schemas, the error sink, alerting, route metrics, the signed cookie, the README parser, the gesture hook's state machine, and five API routes end to end. Seven of them were written because they failed: exploration slots silently unused, an undo that left a ghost in the taste vector, a reaction that counted as a visit, sentence punctuation glued onto repo names, a hard README edge vetoed by a low cosine, a snapshot patch that could drop another process's write, and a pointer the browser took back mid-drag that read as a tap and opened the deep dive. The feed component is split five ways (card, rail, gestures, hint, voice wiring) so an edit to one cannot silently break another, which is how the breath animation and a render bug were lost before. Accessibility was done as a pass, not a sprinkle: AA contrast, one focus ring, global reduced motion, a deck that is a Tab stop with its shortcuts in the accessibility tree and announces each card as a live region, and a deep dive that is a focus-managed dialog.

**Not yet.** There are no device tests, and no lint. Metrics are per-day buckets in Redis, not a time series; there is no dashboard, only the health endpoint and the webhook. Legacy unsigned cookies are still accepted and re-signed; that door closes after a cookie lifetime. The corpus still lacks many named peers until the alternatives crawl runs. The 40/60 blend and every threshold in related-project scoring are calibrated by eye on one corpus.
