...
**The card is the context.** Tap the bars and you are talking to a guide that already knows it: the server bakes the full brief, *why it is in your feed*, six similar repos and a compact profile into the session `instructions` when it mints the ephemeral secret. The browser speaks WebRTC to `gpt-realtime` directly; the key never leaves the server. It opens with a ≤20-second take and a choice, then it is a conversation. Swipes keep working underneath; a page change injects the new brief.
