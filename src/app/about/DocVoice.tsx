...
function Inner() {
  ...
  return (
    <span className="doc-voice">
      <button
        className={`icon-btn feed-search doc-voice-btn${on ? ` on ${voice.state}` : ""}`}
      ...
      {on && (
        <button className={`icon-btn feed-search doc-mute${voice.muted ? " off" : ""}`} onClick={voice.toggleMute} aria-label="Mute microphone" aria-pressed={voice.muted} title={voice.muted ? "Unmute" : "Mute"}>
