...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  // The stack is for you, on your phone, in dev. In production it names files and functions to strangers; the digest is enough to find it in `pnpm errors`.
  const stack = process.env.NODE_ENV === "production" ? "" : (error.stack ?? "").split("\n").slice(0, 6).join("\n");
  return (
    <main style={{ minHeight: "100dvh", padding: "calc(24px + env(safe-area-inset-top)) 22px 40px", background: "#0b0b0f", color: "#e8e8ec", fontFamily: "-apple-system, system-ui, sans-serif" }}>
      ...
      {stack && <pre style={{ fontSize: 11.5, lineHeight: 1.45, color: "#a8a8b0", background: "rgba(255,255,255,.05)", borderRadius: 12, padding: "12px 14px", overflowX: "auto", whiteSpace: "pre-wrap", overflowWrap: "anywhere", margin: "0 0 20px" }}>{stack}</pre>}
